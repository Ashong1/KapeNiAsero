                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ $order->status === 'voided' ? 'opacity-50' : '' }}">
                            ...
                            <td><div class="fw-bold text-dark" style="font-size: 0.9rem;">{{ $order->created_at->format('M d, Y') }}</div><div class="small text-secondary" style="font-size: 0.75rem;">{{ $order->created_at->format('h:i A') }}</div></td>
                            <td><div class="d-flex align-items-center gap-2"><div class="rounded-circle bg-light d-flex align-items-center justify-content-center text-secondary" style="width:24px;height:24px;font-size:0.7rem;"><i class="fas fa-user"></i></div><span class="small fw-bold text-secondary">{{ $order->user->name ?? 'System' }}</span></div></td>
                            <td><span class="fw-bold text-dark fs-6">₱{{ number_format($order->total_price, 2) }}</span></td>
                            <td>
                                @if($order->status == 'completed') <span class="status-badge status-completed"><i class="fas fa-check-circle me-1"></i> Paid</span>
                                @elseif($order->status == 'void_pending') <span class="status-badge status-void-pending"><i class="fas fa-clock me-1"></i> Void Req.</span>
                                @elseif($order->status == 'voided') <span class="status-badge status-voided"><i class="fas fa-ban me-1"></i> Voided</span>
                                @endif
                            </td>
                            <td class="text-end pe-4">
                                <div class="d-flex justify-content-end align-items-center">
                                    <a href="{{ route('orders.receipt', $order->id) }}" class="btn-icon btn-icon-print" target="_blank" title="Print Receipt"><i class="fas fa-print"></i></a>
                        ...
                        @empty
                        <tr><td colspan="6" class="text-center py-5"><div class="text-muted opacity-50"><i class="fas fa-receipt fa-3x mb-3"></i><p class="fw-medium mb-0">No order history found.</p></div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
