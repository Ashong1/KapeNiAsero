...
<style>
    ...
    .success-animation { animation: popIn 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
    @keyframes popIn { 0% { transform: scale(0); opacity: 0; } 100% { transform: scale(1); opacity: 1; } }
    @media (max-width: 991px) { body { overflow: auto !important; height: auto !important; } .pos-layout { flex-direction: column; overflow: visible; height: auto; padding-bottom: 80px; } .product-section { height: auto; overflow: visible; } .product-scroll-area { overflow: visible; padding: 1.5rem; } .cart-section { width: 100%; position: fixed; bottom: 0; left: 0; height: auto; transform: translateY(calc(100% - 85px)); transition: transform 0.3s cubic-bezier(0.4, 0, 0.2, 1); border-top-left-radius: 24px; border-top-right-radius: 24px; box-shadow: 0 -5px 30px rgba(0,0,0,0.15); border-left: none; } .cart-section.expanded { transform: translateY(0); height: 85vh; } .cart-body { padding-bottom: 2rem; } }
</style>
@endsection
    ...
    <div class="product-section">
        <div class="pos-header">
            ...
            <div class="category-scroll">
                <div class="cat-pill active" onclick="filterCategory('all', this)"><i class="fa-solid fa-layer-group"></i> All Items</div>
        ...
        <div class="product-scroll-area">
            <div class="product-grid" id="productGrid">
                ...
                <div class="coffee-card product-item" data-name="{{ strtolower($product->name) }}" data-category="{{ $product->category_id }}" @if(Auth::user()->role != 'admin') onclick="addToCart({{ $product->id }}, '{{ $product->name }}', {{ $product->price }})" @endif>
                    <div class="card-img-box">
                        @if($product->image_path) <img src="{{ asset('storage/' . $product->image_path) }}" class="img-fluid" style="height: 100px; object-fit: contain;">
                        @else <i class="fas fa-mug-hot fa-3x text-secondary opacity-25"></i> @endif
                    </div>
                        ...
                        <div class="d-flex justify-content-between align-items-end mt-auto">
                            ...
                            @if(Auth::user()->role == 'admin')
                                <div class="d-flex gap-2">
                                    <a href="{{ route('products.edit', $product->id) }}" class="btn-admin-icon btn-edit" title="Edit" onclick="event.stopPropagation()"><i class="fa-solid fa-pen-to-square"></i></a>
                                    {{-- UPDATED: Use global confirmDelete helper --}}
                                    <form action="{{ route('products.destroy', $product->id) }}" method="POST" class="d-inline" onsubmit="event.stopPropagation(); confirmDelete(event);">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="btn-admin-icon btn-delete" title="Delete" onclick="event.stopPropagation()"><i class="fa-solid fa-trash-can"></i></button>
                                    </form>
                                </div>
                            @else
                                <div class="rounded-circle bg-light text-primary-coffee d-flex align-items-center justify-content-center shadow-sm" style="width: 28px; height: 28px;"><i class="fa-solid fa-plus fa-xs"></i></div>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div id="noResults" class="text-center py-5 d-none"><i class="fa-solid fa-magnifying-glass fa-3x text-muted opacity-25 mb-3"></i><h6 class="text-secondary">No items found</h6></div>
        </div>
    </div>
    ...
    <div class="cart-section" id="cartSection">
        <div class="cart-header" onclick="toggleCart()">
            <div class="d-flex align-items-center gap-2">
                ...
                <div class="bg-primary-coffee text-white rounded-circle d-flex align-items-center justify-content-center fw-bold ms-2" style="width: 24px; height: 24px; font-size: 0.75rem;" id="cart-badge-count">0</div>
            </div>
            <div class="d-flex align-items-center gap-3">
                {{-- UPDATED: Clear Cart --}}
                <button class="btn btn-sm btn-light text-danger border-0 p-1" onclick="event.stopPropagation(); clearCart()" title="Clear Cart"><i class="fa-solid fa-trash-can"></i></button>
                <i class="fas fa-chevron-up d-lg-none text-secondary" id="cartToggleIcon"></i>
            </div>
        </div>
        <div class="cart-body" id="cart-items">
            <div class="h-100 d-flex flex-column align-items-center justify-content-center text-center text-muted opacity-50"><i class="fa-solid fa-basket-shopping fa-4x mb-3"></i><p class="fw-medium">Start adding items</p></div>
        </div>
        <div class="cart-footer">
            <div class="d-flex justify-content-between mb-2 small text-secondary"><span>Subtotal</span><span class="fw-bold text-dark" id="subtotal">₱0.00</span></div>
            <div class="d-flex justify-content-between mb-3 small text-secondary"><span>VAT (12%)</span><span class="fw-bold text-dark" id="tax">₱0.00</span></div>
            <button class="btn-checkout" onclick="openCheckoutModal()"><span><i class="fa-solid fa-file-invoice-dollar me-2"></i> Charge</span><span id="grand-total">₱0.00</span></button>
        </div>
    </div>
    @endif
</div>
...
<div class="modal fade" id="checkoutModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content modal-content-premium">
            <div class="modal-header-premium"><h5 class="fw-bold m-0"><i class="fa-solid fa-cash-register me-2 text-primary-coffee"></i>Confirm Payment</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
            <div class="modal-body p-4">
                <div class="text-center mb-4"><small class="text-uppercase text-secondary fw-bold">Total Amount</small><div class="display-amount" id="modalTotalAmount">₱0.00</div></div>
                <div class="mb-3">
                    <label class="form-label">Cash Received</label>
                    <div class="input-group"><span class="input-group-text bg-white border-end-0 text-secondary fw-bold" style="border-radius: 16px 0 0 16px;">₱</span><input type="number" id="cashInput" class="form-control input-tendered border-start-0" placeholder="0.00" style="border-radius: 0 16px 16px 0;"></div>
                ...
                <div class="change-display"><div class="small fw-bold text-success opacity-75 text-uppercase">Change</div><div class="fs-4 fw-bold" id="changeAmount">₱0.00</div></div>
            </div>
            <div class="modal-footer border-0 p-4 pt-0">
                ...
                <button type="button" class="btn btn-pos w-100 py-3 rounded-4 fw-bold fs-5 shadow-sm" onclick="confirmPayment()">Complete Payment <i class="fa-solid fa-arrow-right ms-2"></i></button>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content modal-content-premium text-center p-4">
            <div class="modal-body">
                <div class="mb-4"><div class="success-animation"><i class="fa-solid fa-circle-check text-success" style="font-size: 5rem;"></i></div></div>
...
<script>
    ...
    function applyFilters() {
        ...
        items.forEach(item => {
            ...
            if (matchesSearch && matchesCat) { item.style.display = ''; visibleCount++; } else { item.style.display = 'none'; }
        });
    ...
    // --- ALERT UPGRADE 1: Clear Cart ---
    function clearCart() {
        if(cart.length === 0) return;
        Swal.fire({
            title: 'Clear Order?',
            text: "Remove all items from the current cart?",
            icon: 'warning',
    ...
    // --- ALERT UPGRADE 2: Remove Item Confirmation ---
    function updateQty(index, change) {
        if (cart[index].quantity + change <= 0) {
            Swal.fire({
                title: 'Remove Item?',
                text: `Remove ${cart[index].name} from order?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#D32F2F',
                cancelButtonColor: '#86868B',
            ...
            cart[index].quantity += change; renderCart();
        }
    }
    ...
    function renderCart() {
        ...
        cart.forEach((item, index) => {
            ...
            html += `<div class="cart-item animate__animated animate__fadeInRight animate__faster"><div style="min-width: 0; flex: 1;"><div class="fw-bold text-dark text-truncate">${item.name}</div><div class="small text-muted">₱${item.price}</div></div><div class="d-flex align-items-center gap-3 ms-2"><div class="qty-control"><button class="btn-qty" onclick="event.stopPropagation(); updateQty(${index}, -1)"><i class="fa-solid fa-minus"></i></button><span class="fw-bold small" style="min-width:20px; text-align:center;">${item.quantity}</span><button class="btn-qty" onclick="event.stopPropagation(); updateQty(${index}, 1)"><i class="fa-solid fa-plus"></i></button></div><div class="fw-bold text-primary-coffee text-end" style="width: 60px;">₱${itemTotal.toFixed(0)}</div></div></div>`;
        });
        container.innerHTML = html;
        updateTotals(cart.reduce((acc, item) => acc + (item.price * item.quantity), 0));
    }
    ...
    function openCheckoutModal() {
        if(cart.length === 0) {
            Swal.fire({ icon: 'info', title: 'Cart is Empty', text: 'Please add items before charging.', confirmButtonColor: '#6F4E37' });
            return;
        }
    ...
    function calculateChange() {
        ...
        if (change >= 0) {
            ...
            changeDisplay.classList.remove('text-danger'); changeDisplay.classList.add('text-success');
        } else {
            changeDisplay.innerText = 'Insufficient';
            changeDisplay.classList.add('text-danger'); changeDisplay.classList.remove('text-success');
        }
    }
    ...
    function confirmPayment() {
        ...
        if (cash < currentTotal) {
            Swal.fire({
                ...
                text: `You need ₱${(currentTotal - cash).toFixed(2)} more.`,
                confirmButtonColor: '#D32F2F'
            });
            return;
        }
        ...
        fetch('/checkout', {
            ...
            body: JSON.stringify({ cart: cart, cash_tendered: cash })
        })
        .then(res => res.json())
        .then(data => {
            if(data.success) {
                ...
                Swal.fire({ icon: 'error', title: 'Transaction Failed', text: data.message });
            }
        });
    }

    function printReceipt() {
        if(lastOrderId) { window.open('/orders/' + lastOrderId + '/receipt', '_blank'); }
        finishTransaction();
    }
