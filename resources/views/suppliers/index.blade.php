...
<div class="container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('suppliers.create') }}" class="btn btn-primary-coffee btn-action shadow-sm">
            <i class="fas fa-plus-circle"></i> Add Supplier
        </a>
    </div>

    {{-- The Global Layout already handles success/error alerts --}}

    <div class="card card-custom">
        <div class="card-header bg-white border-bottom p-4">
            <h5 class="fw-bold m-0 text-dark">
                <i class="fas fa-truck me-2 text-primary-coffee"></i> Supplier Database
            </h5>
        </div>
            ...
            <table class="table table-hover mb-0 align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="mailto:{{ $supp->email }}" class="text-decoration-none text-secondary hover-primary">
                                <i class="far fa-envelope me-1"></i> {{ $supp->email }}
                            </a>
                        </td>
                        <td class="text-secondary font-monospace">{{ $supp->phone }}</td>
                        <td class="text-end pe-4">
                            <a href="{{ route('suppliers.edit', $supp->id) }}" class="btn btn-sm btn-light text-primary me-1 border" title="Edit">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('suppliers.destroy', $supp->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete supplier? This action cannot be undone.')">
                                @csrf 
                                @method('DELETE')
